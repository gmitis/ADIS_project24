...
tables_definition="$HOME/ADIS_project24/datasources/table_population_scripts/postgresql"
output_file="$HOME/ADIS_project24/datasources/db_population_scripts/postgres_populate.sql"
init_script="$HOME/ADIS_project24/datasources/db_initialization_scripts/postgres_init.sql"
...
create_population_script () {
    ...
    for arg in "$@"; do
        cat "$tables_definition"/"$arg".sql >> "$output_file"
        printf "\n" >> "$output_file"
    done

    echo "Population script for postgresql generated successfully"
}
